...
def prepare_octree(ds, ile, fle=[0.,0.,0.], fre=[1.,1.,1.], ad=None,
                   start_level=0, max_level=None, debug=True):
    if ad is None:
    ...
    octs_dic['LeftEdge'], octs_dic['dx'], octs_dic['Level'], octs_dic['Fields'] = \
        depthFirstHilbert.fill_octree_arrays(ad, fields, total_octs, LeftEdge, dx, Level, Fields)
...
#    ids      = np.zeros(8*nocts_enclosed, dtype='uint64')
    ...
    pbar.finish()
    '''                               
    pos = depthFirstHilbert.position_c()
    hs  = depthFirstHilbert.hilbert_state_c()   
    pbar = get_pbar("Building Hilbert Depth First Octree (ETA is an over-estimate, take as upper limit)",
                    8*nocts_enclosed)
    start_time = time.time()
    depthFirstHilbert.RecurseOctreeDepthFirstHilbert_c(ile,
                                                       -1, #we start on the root grid
                                                       pos,
                                                       hs, 
...
# The commented code below has been rewritten for optimization in the octree_to_depthFirstHilbert
# cython module
'''
